...
export default function AddressScreen() {
  ...
  const geocodeAndPreview = async () => {
    ...
    try {
      ...
      const next: Region = { latitude, longitude, latitudeDelta: 0.012, longitudeDelta: 0.012 };
  ...
  const useCurrentLocation = async () => {
    ...
    try {
      ...
      try {
        ...
        if (r?.[0]) {
          const a = r[0];
          const line = [a.name, a.street, a.city, a.region, a.postalCode, a.country].filter(Boolean).join(', ');
          setAddress(line);
        }
      } catch {}
    } catch {
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.inputs}>
        ...
        <View style={styles.row}>
          <TouchableOpacity onPress={geocodeAndPreview} style={styles.btn} disabled={busy}>
            <Text style={styles.btnText}>{busy ? 'Working…' : 'Geocode & Preview'}</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={useCurrentLocation} style={[styles.btn, styles.ghost]} disabled={busy}>
            <Text style={[styles.btnText, styles.ghostText]}>Use current location</Text>
          </TouchableOpacity>
        </View>
      </View>
      ...
      <TouchableOpacity onPress={save} style={styles.save} disabled={!pin || busy}>
        {busy ? <ActivityIndicator /> : <Text style={styles.saveText}>Save Address</Text>}
      </TouchableOpacity>
    </SafeAreaView>
  );
}
